...
/* 01 — cover: HERO layout, budget-centered (kicker + title + closer).
   heroLayout computes a y_start so the whole stack is vertically centered;
   each take() returns the block top and still guards the footer rail. */
{
  const s = B.newSlide();
  B.glow(s, 12.6, -0.4, 9, "cyan");
  const hero = B.heroLayout([
    [0.38, 0.30], // kicker
    [1.40, 0.20], // title
    [0.50, 0.00], // closer
  ]);
  B.kicker(s, "Sample · slide-quick", hero.take(0.38, 0.30, "kicker"));
  s.addText([
    { text: "The native engine ", options: { color: T.ink } },
    { text: "works", options: { color: T.accentText } },
  ], { x: B.MX, y: hero.take(1.40, 0.20, "title"), w: 11, h: 1.4, fontFace: T.fonts.head, fontSize: 54, bold: true, margin: 0 });
  B.closer(s, [{ text: "Four slides exercising every helper.", options: {} }], hero.take(0.50, 0, "closer"));
  B.footer(s, DECK, 1, 4);
}

/* 02 — comparison: CONTENT layout. A cursor advances title -> panel row and
   refuses to let the panel cross CONTENT_MAX_Y (a build-time overflow guard). */
{
  const s = B.newSlide();
  B.glow(s, -0.3, 6.8, 6.5, "teal");
  B.kicker(s, "01 · Comparison");
  const c = B.cursor();              // starts at CONTENT_TOP, caps at CONTENT_MAX_Y
  const titleY = c.take(1.0, 0.35, "title");
  B.title(s, [{ text: "Panels hold bulleted comparisons", options: {} }], titleY);
  const ph = 2.6, pw = 5.55;
  const py = c.take(ph, 0, "panel-row"); // throws here if the row would overflow
