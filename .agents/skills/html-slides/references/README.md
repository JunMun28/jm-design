...
| `runtime/` | `html-template.md`, `svg-charts.md`, `animation-patterns.md` | Always read `html-template.md` for the controller. `svg-charts.md` now defines chart runtime choice: ECharts for non-trivial charts, inline SVG only for tiny static primitives. `animation-patterns.md` when motion decisions are open. |
